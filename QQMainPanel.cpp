    ...
    msg_center_(new QQMsgCenter(msg_tip_))
{
    ...
    connect(msg_center_, SIGNAL(friendChatMsgArrive(QString)), this, SLOT(changeRecentList(QString)));
...
void QQMainPanel::changeRecentList(QString id)
{
    QQItem *item = findRecentListItemById(id);
    if (!item)
    {
...
void QQMainPanel::setupLoginStatus()
{
    ...
    ui->cb_status_->addItem(QIcon("images/status/imoffline.png"), tr("Offline"), QVariant::fromValue<FriendStatus>(kOffline));
